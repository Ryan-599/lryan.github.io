...
1. 二维装箱问题简介  
   二维装箱问题（2D-BPP）是一个经典的组合优化问题。该问题需要求解将一个矩形序列$(R_1,R_2,...,R_n)$中的每个$R_i$尽可能地、互不重叠地放入多个宽高固定的二维矩形箱中，要求所需矩形箱的数量尽可能少。该问题被证明是一个NP-hard问题。
   该问题可以被数学建模如下：
   - 给定集合$N={1,2,...,n}$，代表n个待装入的目标矩形，$w_j,h_j,a_j=w_jh_j$分别代表第i个目标矩形的宽、高和面积。
   - 给定集合$B={1,2,...,m}$，代表m个箱子，这些箱子有相同的宽、高和面积$W,H,A=WH$。
   - 给定二值变量$y_j$和$x_{ij}$，其中，$y_i$取值为1当且仅当第i个箱子是打开的（即允许放入新的目标矩形）；$x_{ij}$取值为1当且仅当第j个目标矩形被分配给第i个箱子。
